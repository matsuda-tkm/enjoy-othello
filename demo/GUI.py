...
class OthelloMaster:
    ...
    def initOthello(self):
        ...
        # あなたの石の描画位置を計算
        your_init_pos_1_x = NUM_SQUARE // 2 - 1
        your_init_pos_1_y = NUM_SQUARE // 2
        your_init_pos_2_x = NUM_SQUARE // 2
        ...
        for x, y in your_init_pos:
            self.drawDisk(x, y, 'black')

        # 対戦相手の石の描画位置を計算
        com_init_pos_1_x = NUM_SQUARE // 2
        com_init_pos_1_y = NUM_SQUARE // 2
        com_init_pos_2_x = NUM_SQUARE // 2 - 1
        ...
        # COMのターンであればCOMに石を置かせる
        if self.player == COM:
            self.master.after(COM_WAIT_TIME, self.com)
